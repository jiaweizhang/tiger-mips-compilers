...
fun linearize(stm0: T.stm) : T.stm list =
  let
	...
	| reorder nil = (nop,nil)
      ...
      and do_stm(T.SEQ([a])) = do_stm a
	| do_stm(T.SEQ []) = T.LABEL (Temp.newlabel())
	| do_stm(T.SEQ(a::r)) = 
          do_stm a % do_stm (T.SEQ r)
...
fun trace(table,b as (T.LABEL lab :: _),rest) = 
  ...
  (*val _ = print("entering trace for " ^ (Symbol.name lab) ^ "\n")*)
  in case splitlast b
	 ...
	 (case Symbol.look(table, lab)
       ...
       | (most,T.CJUMP(opr,x,y,t,f)) =>
         (case (Symbol.look(table,t), Symbol.look(table,f))
           of (_, SOME(b' as _::_)) => 
              let (*val () = print ("false label ("^(Symbol.name f) ^" found\n")*)
              in 
                  b @ trace(table, b', rest)
              end
            | (SOME(b' as _::_), _) =>
              let (*val () = print ("flipping for true label ("^(Symbol.name f) ^"\n")*)
              in 
		  ...
		  @ trace(table, b', rest)
            ...
            | _ => let 
		val f' = Temp.newlabel()
            (*val () = print ("Making intermediate label" ^ (Symbol.name f') ^ " for " ^ (Symbol.name t) ^"/" ^ (Symbol.name f) ^"\n")*) 
